...
module.exports = {
  ...
  setParser(value) {
    switch (value) {
      case 'temperature': {
        return this.node.endpoints[this.getClusterEndpoint('colorControl')].clusters.colorControl.moveToColorTemperature({
    ...
    switch (value) {
      ...
      case 1: {
        ...
        if (typeof currentLightTemperature !== 'number') {
          try {
            currentLightTemperature = await this.node.endpoints[this.getClusterEndpoint('colorControl')].clusters.colorControl.readAttributes('colorTemperatureMireds');
          } catch (err) {
        ...
        if (typeof currentHue !== 'number') {
          try {
            currentHue = await this.node.endpoints[this.getClusterEndpoint('colorControl')].clusters.colorControl.readAttributes('currentHue');
          } catch (err) {
        ...
        if (typeof currentSaturation !== 'number') {
          try {
            currentSaturation = await this.node.endpoints[this.getClusterEndpoint('colorControl')].clusters.colorControl.readAttributes('currentSaturation');
          } catch (err) {
