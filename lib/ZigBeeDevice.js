...
const DEFAULT_INTERVAL_PING_CHECK_SEC = 600;
const DEFAULT_COUNT_PING_CHECK = 2;
const DEFAULT_CLUSTER_PING_CHECK = CLUSTER.BASIC;
const DEFAULT_ATTRIBUTE_PING_CHECK = 'modelId';
...
class ZigBeeDevice extends Homey.Device {
  ...
  async startOfflinePing(
    intervalSeconds = DEFAULT_INTERVAL_PING_CHECK_SEC,
    retryCount = DEFAULT_COUNT_PING_CHECK,
    cluster = DEFAULT_CLUSTER_PING_CHECK,
    attribute = DEFAULT_ATTRIBUTE_PING_CHECK,
  ) {
    ...
    if (typeof endpoint !== 'number') {
      this.error(`Error: missing cluster ${cluster.NAME}`);
      throw new Error('missing_cluster');
    }
  ...
  /**
   * Stops monitoring a offline with ping.
   */
  stopPingOfflineMonitor() {
    if (this.pingOfflineTimer == null) return;
    this.homey.clearInterval(this.pingOfflineTimer);
  ...
  /**
   * Starts a monitor with reporting attribute
   * Uses configured attribute reporting.
  ...
  async configureOfflineAttributeReporting(
    ...
    this.attributeReportingCheckInterval = intervalSeconds * retryCount
      + SLEEPY_DEVICE_INTERVAL_MARGIN_SEC;
    await this._attachOnlineCheckListener();
