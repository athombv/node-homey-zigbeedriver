...
class ZigBeeDevice extends Homey.Device {
  ...
  onDeleted() {
    ...
    this._detachOnlineCheckListener();
  ...
  async markDeviceOnline() {
    ...
    if (this.deviceOfflineTimer) {
      this.configureOfflineTimeout(this.attributeReportingCheckInterval);
    }
  }
  ...
  async _attachOnlineCheckListener() {
    if (!this._isAttachOnlineListener) {
      ...
      this.originalHandleFrame = this.node.handleFrame;
      ...
      this.node.handleFrame = async (endpointId, clusterId, frame, meta) => {
        if (typeof this.originalHandleFrame === 'function') {
          this.originalHandleFrame.call(this.node, endpointId, clusterId, frame, meta);
        }
        this.markDeviceOnline().catch(err => {
          this.error('Error in markDeviceOnline:', err);
        });
  ...
  async _detachOnlineCheckListener() {
    if (this._isAttachOnlineListener) {
      ...
      this.log('Restored original frame handler');
      this.node.handleFrame = this.originalHandleFrame;
  ...
  async configureOfflineAttributeReporting(
    ...
    this.assertIntervals(intervalSeconds);
    this.assertRetryCount(retryCount);
    assertZCLNode(this.zclNode, endpoint, reportingCluster);
    ...
    this.attributeReportingCheckInterval = intervalSeconds * retryCount
      + INTERVAL_MARGIN_SECONDS;
    this.configureOfflineTimeout(this.attributeReportingCheckInterval);
  }
   ...
   */
  configureOfflineTimeout(intervalSeconds) {
    this.assertIntervals(intervalSeconds);
    this.stopOfflineMonitor();
    this._attachOnlineCheckListener();
