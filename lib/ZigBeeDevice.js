...
class ZigBeeDevice extends MeshDevice {
  ...
  _initNodeListeners() {
    ...
    this.node.on('leave', () => { // TODO: this can be removed? handled by manager zigbee now?
      this.setUnavailable(this.__(i18n.error.left_network));
    });

    // Start listening for incoming reports
    this.node.on('report', report => { // TODO: this does not work like this anymore
  ...
  _registerCapabilityGet(capabilityId, clusterId) {
    ...
    if (capabilityGetObj.opts.getOnStart
      ...
      && !(this.node.battery && this.node.online === false && capabilityGetObj.opts.getOnOnline === true)) { // TODO: these properties are different
      this._getCapabilityValue(capabilityId, clusterId);
    }

    if (capabilityGetObj.opts.getOnOnline) {
      this.node.on('online', () => { // TODO: this is different
  ...
  _registerCapabilitySet(capabilityId, clusterId) {
    ...
    this.registerCapabilityListener(capabilityId, async (value, opts) => this._registerCapabilityListenerHandler(capabilitySetObj, capabilityId, value, opts));
  }
  ...
  _mergeSystemAndUserOpts(capabilityId, clusterId, userOpts) {
    ...
    try {
      ...
      systemOpts = Homey.util.recursiveDeepCopy(require(`./system/capabilities/${capabilityId}/${clusterId}.js`)); // TODO: fix this
    ...
    if (userOpts && !Object.prototype.hasOwnProperty.call(userOpts, 'endpoint')) {
      _userOpts = { ...userOpts, endpoint: this.getClusterEndpoint(clusterId) };
  ...
  registerReportListener(clusterId, commandId, triggerFn, endpointId = 0) {
    ...
    if (alreadyBound || alreadyRegistered) {
      ...
      this._bindRequests.push({
        endpointId, clusterId, commandId, reportId,
      });
  ...
  async _bindCluster() {
    ...
    try {
      await this.zclNode.endpoints[bindObj.endpointId].clusters[bindObj.clusterId].bind(); // TODO: implement new bindings (JV)
    } catch (err) {
  ...
  async registerAttrReportListener(clusterId, attrId, minInt, maxInt, repChange = null, triggerFn, endpointId = 0) {
    ...
    if (!this.zclNode.endpoints[endpointId] || !this.zclNode.endpoints[endpointId].clusters[clusterId]) {
  ...
  async _configureReport() {
    ...
    try {
      ...
      await this.zclNode.endpoints[configureReportObj.endpointId].clusters[configureReportObj.clusterId]
        .configureReporting({
  ...
  printNode() {
    ...
    this.log('- Battery:', this.zclNode.battery); // TODO: changed?

    Object.keys(this.zclNode.endpoints).forEach(endpointsId => {
      this.log('- Endpoints:', endpointsId);
      this.log('-- Clusters:');
      Object.keys(this.zclNode.endpoints[endpointsId].clusters).forEach(key => {
        this.log('---', key);
        if (typeof this.zclNode.endpoints[endpointsId].clusters[key].attrs !== 'undefined') { // TODO: changed?
          Object.keys(this.zclNode.endpoints[endpointsId].clusters[key].attrs).forEach(attrKey => {
            this.log('----', attrKey, ':', this.zclNode.endpoints[endpointsId].clusters[key].attrs[attrKey]);
          });
        }
      });
    });
