...
/**
 * @typedef {string} ClusterName - Zigbee {@link Cluster.NAME} (e.g. 'basic' for
 * {@link BasicCluster})
 */
...
class ZigBeeDevice extends Homey.Device {
   ...
   * @param {ClusterName|ClusterId} cluster - Cluster name (e.g. 'onOff') or cluster id (e.g. 4 for
   * 'onOff').
   * @returns {EndpointId} endpointId
   */
  getClusterEndpoint(cluster) {
    if (typeof cluster !== 'number' && typeof cluster !== 'string') {
      throw new TypeError('expected_cluster_string_or_number');
    }
    ...
    for (const [endpointId, endpoint] of Object.entries(this.zclNode.endpoints)) {
      if (endpoint.clusters
        && (endpoint.clusters[getClusterName(cluster)] || endpoint.clusters[cluster])
      ) {
