...
const SW_BUILD_ID = '__swBuildId';

// Constants for offline monitoring
const DEFAULT_PING_INTERVAL_SECONDS = 600;
const DEFAULT_PING_RETRY_COUNT = 2;
const DEFAULT_CLUSTER_PING = CLUSTER.BASIC;
const DEFAULT_ATTRIBUTE_PING = 'modelId';
const UNSET_ENDPOINT = -1;
const INTERVAL_MARGIN_SECONDS = 30;
...
/**
 * Handle frame event key.
 * @constant
 * @type {string}
 * @private
 */
const HANDLE_FRAME_EVENT = 'frame';
...
class ZigBeeDevice extends Homey.Device {
   ...
   * Note that when configured for a product id all relevant energy settings must be provided,
   * as it fully overrides the energy object configured in the driver manifest.
   * @abstract
  ...
  /**
   * If the {@link energyMap} object is available and has an entry for the `productId` of this
   * device this entry will override the energy object configured in the drivers' manifest.
   */
  async updateEnergyConfiguration() {
    // Check if `getEnergy` method is available (Homey >=v3.0.0)
    if (typeof this.getEnergy !== 'function') {
      return;
    }

    // Check whether an explicit energy map has been defined
    const zigbeeProductId = this.getSetting('zb_product_id');
    if (!zigbeeProductId || !this.energyMap[zigbeeProductId]) {
      return;
    }

    // Merge energyMap data with existing energy overrides
    const energyConfig = {
      ...this.getEnergy(),
      ...this.energyMap[zigbeeProductId],
    };
    this.debug('Updating energy configuration', energyConfig);
    await this.setEnergy(energyConfig);
  }
        ...
        configurationPromises.push(
          wrapAsyncWithRetry( // Wrap with retry (2-time, directly)
            ...
            .then(async () => {
              ...
              const currentConfigurations = this.getStoreValue(
                CONFIGURED_ATTRIBUTE_REPORTING_STORE_KEY,
              ) || [];

              // Add last updated property to attribute reporting configuration object and restore
              // previously removed attributes
              let filteredConfigurations = currentConfigurations;
              for (const [attributeName, obj] of Object.entries(
                ...
                // Remove elements from array that match the endpoint, cluster and attribute
                // name to prevent duplicate configurations (only one config can be active
                // for a specific endpoint, cluster and attribute, so store the latest).
                filteredConfigurations = filteredConfigurations.filter(
                  x => {
                    const config = x[attributeName];
                    if (config != null && config.endpointId === endpointId
                ...
                if (this.deviceOfflineTimer) {
                  this.reConfigureOfflineAttributeReporting(obj.maxInterval,
                    endpointId,
                    clusterName,
                    attributeName);
                }
  ...
  async onInit() {
    ...
    this._flowTriggers = {};
    this._isAttachFrameListener = false;
    ...
    this.homey.zigbee.getNode(this)
      .then(async node => {
        ...
        await this.updateEnergyConfiguration();
  ...
  onDeleted() {
    ...
    this.stopOfflineMonitor();

    this.debug('deleted ZigBeeDevice instance');
  }

  /**
   * Retrieves the `swBuildId` from the Basic cluster of the specified endpoint. If the
   * `swBuildId` is already cached in the device store, it will return the cached value.
   * @param {object} [params={}] Method parameters.
   * @param {number} [params.endpointId=1] The endpoint id to fetch the swBuildId from (default: 1).
   * @param {boolean} [params.skipCache=false] If true, it will skip the cache and always fetch the
   * swBuildId from the device (default: false).
   * @returns {Promise<string>} The swBuildId of the device.
   */
  async getSwBuildId({ endpointId = 1, skipCache = false } = {}) {
    if (skipCache === false) {
      // Check if the zw_sb_build_id is reachable in this Homey version
      // Getting system settings from apps is no longer available after Homey Pro 12.4.8.
      const swBuildIdSetting = this.getSetting('zw_sb_build_id');
      if (typeof swBuildIdSetting === 'string') {
        this.debug(`Using swBuildId from zw_sb_build_id setting: ${swBuildIdSetting}`);
        await this.setStoreValue(SW_BUILD_ID, swBuildIdSetting)
          .catch(err => this.error('Error: could not store swBuildId in device store', err));

        return swBuildIdSetting;
      }

      const cachedVersion = this.getStoreValue(SW_BUILD_ID);
      if (cachedVersion) {
        this.debug(`Found cached swBuildId: ${cachedVersion}`);
        return cachedVersion;
      }
    }

    this.debug('No cached swBuildId found, fetching from node...');
    const swBuildId = await this._fetchSwBuildId({ endpointId });
    this.debug(`Fetched swBuildId: ${swBuildId}`);

    await this.setStoreValue(SW_BUILD_ID, swBuildId)
      .catch(err => this.error('Error: could not store swBuildId in device store', err));
  ...
  /**
   * Fetches the `swBuildId` from the Basic cluster of the specified endpoint.
   * @param {object} params Method parameters.
   * @param {number} params.endpointId The endpoint id to fetch the swBuildId from.
   * @returns {Promise<string>} The swBuildId of the device.
   * @throws {Error} If the endpointId is not a number.
   * @throws {Error} If the zclNode is not available or the swBuildId attribute cannot be read.
   * @throws {Error} If the swBuildId attribute is not found or has an invalid type.
   * @private
   */
  async _fetchSwBuildId({ endpointId }) {
    if (typeof endpointId !== 'number') {
      throw new Error('Invalid endpointId, expected a number');
    }

    if (!this.zclNode) {
      this.error('Error: zclNode is not available, cannot fetch swBuildId');
      throw new Error('zclNode is not available, cannot fetch swBuildId');
    }

    const attributes = await this.zclNode.endpoints[endpointId].clusters.basic.readAttributes(['swBuildId'])
      .catch(err => {
        this.error('Error: could not read swBuildId attribute from Basic cluster', err);
        throw err;
      });

    if (!attributes || typeof attributes.swBuildId !== 'string') {
      this.error('Error: swBuildId attribute not found or invalid type:', attributes);
      throw new Error('Could not find swBuildId attribute on the Basic cluster');
    }

    return attributes.swBuildId;
  }
  ...
  /**
   * Sets up a periodic timer to check if a device is offline.
   * Checks the device's online status by reading an attribute from the cluster.
   * If the read fails repeatedly, the device is marked as offline.
   * @param {number} intervalSeconds - Check interval in seconds. 60 or greater.
   * @param {number} retryCount - Consecutive failures before marking offline.
   * @param {number} endpoint - Endpoint to check.
   * @param {object} cluster - Cluster to read from.
   * @param {string} attribute - Attribute to read.
   */
  async startOfflinePing(
    intervalSeconds = DEFAULT_PING_INTERVAL_SECONDS,
    retryCount = DEFAULT_PING_RETRY_COUNT,
    ...
    this.pingTimeout = async () => {
      if (this.isPingRunning) {
        return;
      }
      this.isPingRunning = true;
      try {
        // Read values from the cluster for ping
        await this.zclNode.endpoints[pingEndpoint]
          .clusters[cluster.NAME].readAttributes([attribute]);
      } catch (err) {
        this.pingRetryCount++;
        this.error(`Offline detection ping failed ${this.pingRetryCount}/${this.retryCount}:`, err);
      }
      if (this.pingRetryCount >= this.retryCount && this.getAvailable()) {
        await this.setUnavailable().catch(() => null);
        this.log(`Device is offline (reason: ${this.pingRetryCount} ping failures)`);
        this.pingRetryCount = 0;
      }
      this.pingOfflineTimer = this.homey.setTimeout(this.pingTimeout, this.pingInterval * 1000);
      this.isPingRunning = false;
    };
    this.pingOfflineTimer = this.homey.setTimeout(this.pingTimeout, this.pingInterval * 1000);
  }
  ...
  async markDeviceOnline() {
    const now = Date.now();
    if (now - this._lastUpdateTime < ONLINE_CHECK_THROTTLE_MS && this.getAvailable()) {
      // Skip execution if the function is called too frequently
      return;
    }
    this._lastUpdateTime = now;
    if (!this.getAvailable()) {
      this.log('Device came back online');
      await this.setAvailable();
    }
    if (this.pingOfflineTimer && this.pingTimeout && !this.isPingRunning) {
      this.stopPingOfflineMonitor();
      this.pingOfflineTimer = this.homey.setTimeout(this.pingTimeout, this.pingInterval * 1000);
    }
    if (this.deviceOfflineTimer) {
  ...
  /**
   * Stops monitoring offline status with ping.
   */
  stopPingOfflineMonitor() {
    if (this.pingOfflineTimer == null) return;
    this.pingRetryCount = 0;
    this.homey.clearTimeout(this.pingOfflineTimer);
    this.pingOfflineTimer = null;
  }

  /**
   * Stops monitoring a timeout for offline status.
   */
  stopOfflineMonitor() {
    if (this.deviceOfflineTimer == null) return;
    this.homey.clearTimeout(this.deviceOfflineTimer);
    this.deviceOfflineTimer = null;
  }

  /**
   * Attaches a frame listener for online check.
   * @private
   */
  _attachFrameListener() {
    if (!this._isAttachFrameListener) {
      this._isAttachFrameListener = true;
      this.node.on(HANDLE_FRAME_EVENT, this.onFrameEvent.bind(this));
    }
  }

  /**
   * Event handler for frame events. This function is called when a frame event is received.
   */
  onFrameEvent() {
    this.markDeviceOnline().catch(err => {
      this.error('Error in markDeviceOnline:', err);
  ...
  /**
   * Starts a monitor with reporting attribute. Uses configured attribute reporting.
   ...
   * @param {number} reportingEndpoint - The Endpoint ID used for reporting.
   * @param {object} reportingCluster - Cluster for attribute reporting.
   * @param {string} reportingAttributeName - Attribute name for reporting.
   */
  async configureOfflineAttributeReporting(
    intervalSeconds,
    retryCount,
    reportingEndpoint,
    reportingCluster,
    ...
    if (!clusterAttributeData || clusterAttributeData.length === 0) {
      ...
      try {
        ...
        this.error('Failed to configure attribute reporting for offline detection', err);
        throw err;
      }
    } else {
      this.log(`Update interval ${intervalSeconds} to ${clusterAttributeData.maxInterval}`);
      intervalSeconds = clusterAttributeData.maxInterval;
    }

    // save configure variable for reconfigure
    this.offlineReportingConfig = {
      reportingEndpoint,
      reportingClusterName: reportingCluster.NAME,
      reportingAttributeName,
      retryCount,
    };

    this.offlineCheckInterval = intervalSeconds * retryCount + INTERVAL_MARGIN_SECONDS;
    this.startOfflineTimeout(this.offlineCheckInterval);
  }

  /**
   * Reconfigures the offline attribute reporting when configure attribute resetting.
   * @param {number} intervalSeconds - The interval in seconds for offline checking.
   * @param {number} endpoint - The endpoint value.
   * @param {number} clusterName - The name of the cluster.
   * @param {string} attributeName - The name of the attribute.
   * @returns {Promise<void>}
   */
  async reConfigureOfflineAttributeReporting(
    intervalSeconds,
    endpoint,
    clusterName,
    attributeName,
  ) {
    if (!this.offlineReportingConfig) return;

    const {
      reportingEndpoint, reportingClusterName,
      reportingAttributeName, retryCount,
    } = this.offlineReportingConfig;
    if (Number(endpoint) !== reportingEndpoint
      || clusterName !== reportingClusterName
      || attributeName !== reportingAttributeName) {
      return;
    }
    this.offlineCheckInterval = intervalSeconds * retryCount + INTERVAL_MARGIN_SECONDS;
    this.log(`reConfigure offline timer, cluster: ${reportingClusterName}, time: ${this.offlineCheckInterval}`);
    this.startOfflineTimeout(this.offlineCheckInterval);
  }

  /**
   * Configures the offline timeout interval and starts the offline timeout process.
   * @param {number} intervalSeconds - Time in seconds until offline.
   */
   ...
   * @private
   */
  startOfflineTimeout(intervalSeconds) {
    this.stopOfflineMonitor();
    this.deviceOfflineTimer = this.homey.setTimeout(async () => {
      this.setUnavailable()
        .then(() => {
          this.log(`Device is offline (reason: offline timer reached ${intervalSeconds}s)`);
        })
        .catch(err => {
          this.error('Error setting device to unavailable', err);
        });
  ...
  /**
   * Validates if the interval is a number and meets the minimum requirement.
   * @param {number} intervalSeconds - The interval in seconds to validate.
   * @private
   */
  assertIntervals(intervalSeconds) {
    if (typeof intervalSeconds !== 'number' || intervalSeconds < MIN_INTERVAL_SECONDS) {
      this.error('Interval is invalid. 60 or greater.');
      throw new Error('interval_invalid');
    }
  }

  /**
   * Validates if the retry count is a positive number.
   * @param {number} retryCount - The retry count to validate.
   * @private
   */
  assertRetryCount(retryCount) {
    if (typeof retryCount !== 'number' || retryCount <= 0) {
      this.error('Retry count is invalid.');
