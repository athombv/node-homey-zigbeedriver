...
class ZigBeeDevice extends Homey.Device {
  ...
  /**
   * Retrieves the `swBuildId` from the Basic cluster of the specified node. If the 
   * `swBuildId` is already cached in the device store, it will return the cached value.
   * @param {number} nodeId The node id to fetch the swBuildId from, defaults to 1.
   * @returns {Promise<string>} The swBuildId of the device.
   */
  async getSwBuildId(nodeId = 1) {
    ...
    const swBuildId = await this._fetchSwBuildId(nodeId);
  ...
  /**
   * Fetches the `swBuildId` from the Basic cluster of the specified node.
   * @param {number} nodeId The node id to fetch the swBuildId from.
   * @returns {Promise<string>} The swBuildId of the device.
   * @throws {Error} If the nodeId is not a number.
  ...
  async _fetchSwBuildId(nodeId) {
    ...
    const attributes = await this.zclNode.endpoints[nodeId].clusters.basic.readAttributes(['swBuildId'])
      .catch(err => {
