...
class ZigBeeDevice extends MeshDevice {
    ...
    if (parsedPayload instanceof Error) throw parsedPayload;
      ...
      return 'IGNORED';
		...
		try {
		  console.log(capabilitySetObj.endpoint)
	...
	_mergeSystemAndUserOpts(capabilityId, clusterId, userOpts) {
		...
		let systemOpts = {};
		try {
			systemOpts = Homey.util.recursiveDeepCopy(require(`./lib/system`));
	...
	getClusterEndpoint(clusterName) {
		...
		if (!this.zclNode || !this.zclNode.hasOwnProperty('endpoints')) return new Error('node_not_initialized');
		...
		for (const [endpointId, endpoint] of Object.entries(this.zclNode.endpoints)) {
			if (endpoint.hasOwnProperty('clusters') && endpoint.clusters.hasOwnProperty(clusterName)) {
				return endpointId;
			}
		}
