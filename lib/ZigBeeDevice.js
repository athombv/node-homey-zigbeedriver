...
class ZigBeeDevice extends Homey.Device {
   ...
   *    return setValue ? 'setOn 'setOff'; // This could also be an object for more complex commands
   *  },
   *  get: 'onOff'
   *  report: 'onOff'
  ...
  printNode() {
    ...
    Object.keys(this.zclNode.endpoints)
      .forEach(endpoint => {
        ...
        this.log('-- Bindings:');
        Object.keys(this.zclNode.endpoints[endpoint].bindings)
          .forEach(key => {
            this.log('---', key);
          });
      });
   ...
   * indication. Finally, if {@link ClusterCapabilityConfiguration.getOpts.pollInterval} is set
   * to either a number or a string (setting key) a poll interval will be registered which
   * executes the cluster command to retrieve the capability value.
   * @param {CapabilityId} capabilityId
   * @param {ClusterSpecification} cluster
   * @private
   */
  ...
  _mergeSystemAndUserClusterCapabilityConfigurations(
    ...
    if (typeof endpoint !== 'number') endpoint = 1; // Default to 1 if it could not be found
