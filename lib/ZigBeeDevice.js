...
class ZigBeeDevice extends Homey.Device {
  ...
  async getClusterCapabilityValue(capabilityId, cluster) {
    ...
    const result = await this.zclNode.endpoints[endpoint].clusters[cluster.NAME].readAttributes([get])
      .catch(err => {
  ...
  async setClusterCapabilityValue(capabilityId, cluster, value, opts = {}) {
    ...
    return this.zclNode.endpoints[endpoint].clusters[cluster.NAME][set](parsedPayload)
      .catch(err => {
  ...
  async onInit() {
    ...
    await this.migrateDeeplyNestedAttributeReportingConfigurationsStore()
      .catch(err => this.error('Error: could not migrate deeply nested attribute reporting configurations store', err));
  ...
  /**
 * This migration is required because a bug that was fixed in
 * https://github.com/athombv/node-homey-zigbeedriver/pull/145/ caused deeply nested configuration
 * objects in the device store.
 *
 * Before: { "0" : { "0" : { "0" : { ... }, "keypadLockout" : { "minInterval" : 0, "maxInterval" :
 * 300, "minChange" : 1, "lastUpdated" : 1762819836810.0, "clusterName" :
 * "thermostatUserInterfaceConfiguration", "attributeName" : "keypadLockout", "endpointId" : "1" }
 * }, "activePower" : { "minInterval" : 10, "maxInterval" : 60000, "minChange" : 0.1, "lastUpdated"
 * : 1762819838834.0, "clusterName" : "electricalMeasurement", "attributeName" : "activePower",
 * "endpointId" : "1" } }, "currentSummationDelivered" : { "minInterval" : 10, "maxInterval" :
 * 60000, "minChange" : 0.001, "lastUpdated" : 1762819839105.0, "clusterName" : "metering",
 * "attributeName" : "currentSummationDelivered", "endpointId" : "1" } }
 *
 * After: [{ "currentSummationDelivered" : { "minInterval" : 10, "maxInterval" : 60000, "minChange"
 * : 0.001, "lastUpdated" : 1762819839105.0, "clusterName" : "metering", "attributeName" :
 * "currentSummationDelivered", "endpointId" : "1" } }]
 *
 * @private
 */
  async migrateDeeplyNestedAttributeReportingConfigurationsStore() {
    const storeValue = this.getStoreValue(CONFIGURED_ATTRIBUTE_REPORTING_STORE_KEY);
    if (!storeValue) return;
    if (!Array.isArray(storeValue)) return;

    // Filter out nested objects, keep only valid flat configs
    const validConfigs = [];
    for (const item of storeValue) {
      if (!item || typeof item !== 'object') continue;

      // Check each property - if it has clusterName/attributeName, it's valid
      for (const key of Object.keys(item)) {
        const config = item[key];
        if (config && typeof config === 'object' && config.clusterName && config.attributeName) {
          validConfigs.push({ [key]: config });
        }
      }
    }

    await this.setStoreValue(CONFIGURED_ATTRIBUTE_REPORTING_STORE_KEY, validConfigs);
  }
