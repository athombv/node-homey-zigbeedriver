...
const {
  __, assertClusterSpecification, assertCapabilityId, assertZCLNode,
} = require('./util');
...
class ZigBeeDevice extends Homey.Device {
   ...
   * TODO: migration: cluster changed from string to number
   ...
   *    return setValue ? 'on': 'off'; // This could also be an object for more complex commands
   *  },
   *  get: 'onOff'
   *  report: 'onOff'
  ...
  registerCapability(capabilityId, cluster, clusterCapabilityConfiguration) {
    ...
    this._debug(`register capability ${capabilityId} with cluster ${cluster.NAME}`);
    ...
    this._debug(`registered capability ${capabilityId} with cluster ${cluster.NAME}, configuration:`, this._getClusterCapabilityConfiguration(capabilityId, cluster));
  ...
  registerMultipleCapabilities(
    ...
    multipleCapabilitiesConfiguration.forEach(capabilityConfiguration => {
      ...
      this._debug(`register multiple capabilities → registered ${capabilityId}, with configuration:`, this._getClusterCapabilityConfiguration(capabilityId, cluster));
      ...
      for (const [clusterName, clusterConfig] of Object.entries(endpointConfig)) {
        ...
        for (const [attributeName, attributeConfig] of Object.entries(clusterConfig)) {
          ...
          if (!this.zclNode.endpoints[endpointId]
            ...
            this.error(`Error: expected cluster ${clusterName} on endpoint ${endpointId}`);
            throw new Error('missing_cluster_or_endpoint');
          }

          assertZCLNode(this.zclNode, endpointId, { NAME: clusterName });

          this._debug(`configure attribute reporting (endpoint: ${endpointId}, cluster: ${clusterName}, attribute: ${attributeName})`, {
            minInterval,
            maxInterval,
        ...
        configurationPromises.push(this.zclNode.endpoints[endpointId]
          .clusters[clusterName]
          .configureReporting(configureAttributeReportingOptions)
          .then(async res => {
            this._debug(`configured attribute reporting (endpoint: ${endpointId}, cluster: ${clusterName})`, configureAttributeReportingOptions);

            // Add last updated property to attribute reporting configuration object and restore
            // previously removed attributes
            for (const [attributeName, obj] of Object.entries(configureAttributeReportingOptions)) {
            ...
            this._debug(`stored attribute reporting configuration (endpoint: ${endpointId}, cluster: ${clusterName})`, this.getStoreValue(CONFIGURED_ATTRIBUTE_REPORTING_STORE_KEY));
            return res;
          })
          .catch(err => {
            this.error(`Error: configuring attribute reporting (endpoint: ${endpointId}, cluster: ${clusterName})`, configureAttributeReportingOptions);
            throw err;
          }));
      }
    }
   ...
   * const { CLUSTER } = require('zigbee-clusters');
   *
   * zclNode.endpoints[1].clusters.onOff.on('attr.onOff', report => {
   *   return this.parseAttributeReport('onoff', CLUSTER.ON_OFF, report);
   * });
   */
  async parseAttributeReport(capabilityId, cluster, payload) {
    ...
    const { reportParser } = this._getClusterCapabilityConfiguration(capabilityId, cluster);
    if (typeof reportParser !== 'function') return null;

    this._debug(`handle report (cluster: ${cluster.NAME}, capability: ${capabilityId}), raw payload:`, payload);

    const parsedPayload = await reportParser.call(this, payload);
  ...
  async getClusterCapabilityValue(capabilityId, cluster) {
    ...
    this._debug(`get → ${capabilityId} → read attribute (cluster: ${cluster.NAME}, attributeId: ${get}, endpoint: ${endpoint}) → raw result:`, result);

    // Parse the raw result
    const parsedResult = this.parseAttributeReport(capabilityId, cluster, result);
  ...
  async setClusterCapabilityValue(capabilityId, cluster, value, opts = {}) {
    ...
    if (parsedPayload === null) {
      this._debug(`WARNING: set ${capabilityId} → ${value} (command: ${set}, cluster: ${cluster.NAME}, endpoint: ${endpoint}) returned \`null\`, ignoring command set`);
      return 'IGNORED'; // TODO: is this the best solution?
    }

    this._debug(`set ${capabilityId} → ${value} (command: ${set}, cluster: ${cluster.NAME}, endpoint: ${endpoint}), parsed payload:`, parsedPayload);
  ...
  onInit() {
    ...
    Homey.ManagerZigBee.getNode(this)
      .then(async node => {
        ...
        this.onNodeInit({ zclNode: this.zclNode, node });
      })
      .catch(err => {
  ...
  onDeleted() {
    ...
    if (this.zclNode) {
      for (const endpoint of this.zclNode.endoints) {
        for (const cluster of endpoint.cluster) {
  ...
  _registerCapabilityGet(capabilityId, cluster) {
    ...
    if (typeof report === 'string') {
      this.zclNode.endpoints[endpoint].clusters[cluster.NAME]
        .on(`attr.${report}`, this.parseAttributeReport.bind(this, capabilityId, cluster));
    }
    ...
    if (getOnStart
      ...
      this.getClusterCapabilityValue(capabilityId, cluster)
        .catch(err => {
          this.error(`Error: could not get value for capability: ${capabilityId} on cluster: ${cluster.NAME}`, err);
        });
    }
    ...
    if (getOnOnline) {
      this.node.on('endDeviceAnnounce', () => {
        ...
        this.getClusterCapabilityValue(capabilityId, cluster)
          .catch(err => {
            this.error(`could not get value for capability: ${capabilityId} on cluster: ${cluster.NAME}`, err);
          });
      });
    }
  ...
  _setPollInterval(capabilityId, cluster, pollInterval) {
    ...
    this._debug(`set poll interval for capability: ${capabilityId}, cluster: ${cluster.NAME} to ${pollInterval}ms`);

    // Set interval
    this._pollIntervals[capabilityId][cluster.ID] = setInterval(() => {
      this._debug(`polling cluster ${cluster.NAME} for capability ${capabilityId}`);
  ...
  _getClusterCapabilityConfiguration(capabilityId, cluster) {
    ...
    if (capabilityConfiguration) {
      const clusterCapabilityConfiguration = capabilityConfiguration.get(cluster.ID) || null;
      if (!capabilityConfiguration) {
  ...
  _setClusterCapabilityConfiguration(capabilityId, cluster, clusterCapabilityConfiguration) {
    ...
    if (clusterCapabilityConfiguration) {
      this._clusterCapabilityConfigurations.set(capabilityId, {
        ...this._clusterCapabilityConfigurations.get(capabilityId),
        // Freeze to avoid modifications
        [cluster.ID]: Object.freeze(clusterCapabilityConfiguration),
      });
    }
  }
