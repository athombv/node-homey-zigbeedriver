...
class ZigBeeDevice extends Homey.Device {
  ...
  onNodeInit({ zclNode, node }) {
    this.unsetStoreValue(SW_BUILD_ID)
      .then(() => this.getSwBuildId())
      .catch(err => this.error('Error updating swBuildId:', err));
  }
   ...
   * @param {number} endpointId The endpoint id to fetch the swBuildId from, defaults to 1.
   * @returns {Promise<string>} The swBuildId of the device.
   */
  async getSwBuildId(endpointId = 1) {
