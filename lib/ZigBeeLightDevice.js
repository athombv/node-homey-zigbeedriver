...
const onoffCapabilityDefinition = {
  ...
  opts: {
    get: ON_OFF,
    reportParser(value) {
      this.debug(`\`onoff\` → reportParser → onOff: ${value} → parsed:`, value);
      return value;
    },
    report: ON_OFF,
    getOpts: {
...
class ZigBeeLightDevice extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    const levelControlCluster = zclNode.endpoints[levelControlClusterEndpoint].clusters
      .levelControl;
    ...
    // If needed make color control cluster available
    ...
    if (this.hasCapability('light_hue')
      ...
      colorControlCluster = zclNode.endpoints[colorControlEndpoint].clusters.colorControl;
    }
