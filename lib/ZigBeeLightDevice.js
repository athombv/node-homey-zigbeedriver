...
class ZigBeeLightDevice extends ZigBeeDevice {
  ...
  async readTransitionTimes() {
    try {
      const attributes = await this.levelControlCluster.readAttributes([
      ...
      if (Number.isFinite(attributes[ON_TRANSITION_TIME])
        && attributes[ON_TRANSITION_TIME] !== TRANSITION_TIME_UNSET) {
        this._onTransitionTime = attributes[ON_TRANSITION_TIME] * 100;
      }
      this.debug('read transition times', {
   ...
   *
   * These reports only arrive if the app asks for them: the cluster has to be listed in
   * `zigbee.endpoints[].bindings` in the driver manifest, and {@link configureAttributeReporting}
   * has to be called for the attribute. Neither is done here on purpose. A binding is a change to
   * the device itself, and reporting intervals are a per-device trade-off between staying in sync
   * and flooding the network, so choosing either for every light could conflict with what the app
   * wants. Devices that do not report simply never trigger these listeners.
   */
  registerAttributeReportListeners() {
    if (this.hasCapability('dim') && this.getClusterEndpoint(CLUSTER.LEVEL_CONTROL) !== null) {
      this._addAttributeReportListener(this.levelControlCluster, `attr.${CURRENT_LEVEL}`, currentLevel => {
        ...
        // A device reports every step of a transition it is running, and every one of those but
        // the last is a level nobody asked for.
        if (Date.now() < this._dimTransitionEndsAt) return;
  ...
  async changeOnOff(onoff) {
    ...
    return this.onOffCluster[onoff ? 'setOn' : 'setOff']()
      .then(async result => {
        if (onoff === false) {
          this._dimTransitionEndsAt = 0; // Turning off ends any dim transition
          this.setCapabilityValue('dim', 0).catch(this.error); // Set dim to zero when turned off
        } else if (onoff) {
          // Wait for the device to settle, it ramps to its on level over `onTransitionTime` and
          // does not update `currentLevel` instantly either way.
          ...
          // asked for. Checked again after the read, a dim command can still come in while it is
          // in flight.
          const dimCommandWins = () => this._dimCommandAt > onOffCommandAt - DIM_COMMAND_GRACE
            || Date.now() < this._dimTransitionEndsAt;
          this.homey.clearTimeout(this._dimReadbackTimeout);
          ...
          }, Math.max(DIM_READBACK_DELAY, this.onCommandTransitionTime));
        }
        return result;
      });
  }
  ...
  async changeDimLevel(dim, opts = {}) {
    ...
    this._dimTransitionEndsAt = this._dimCommandAt
  ...
  async onEndDeviceAnnounce() {
    ...
    if (this._onOffTransitionTime === null) this.readTransitionTimes();
