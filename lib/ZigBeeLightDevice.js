...
class ZigBeeLightDevice extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode, supportsHueAndSaturation, supportsColorTemperature }) {
    ...
    // Read attribute values from device on first init
  ...
  async readColorControlAttributes() {
    ...
    return this.colorControlCluster.readAttributes(
      ...
      .then(async ({
        ...
        await Promise.all([
          this.setStoreValue('colorCapabilities', colorCapabilities),
          this.setStoreValue('colorTempMin', colorTempPhysicalMinMireds),
          this.setStoreValue('colorTempMax', colorTempPhysicalMaxMireds),
          this.setStoreValue('colorClusterConfigured', true),
        ]);
  ...
  async changeDimLevel(dim, opts = {}) {
    ...
    const moveToLevelWithOnOffCommand = {
      ...
      transitionTime: calculateDimDuration(opts, this.getSettings()),
    };
  ...
  async onEndDeviceAnnounce() {
    const {
      ...
      colorMode,
      colorTemperatureMireds,
    } = await this.colorControlCluster.readAttributes(
