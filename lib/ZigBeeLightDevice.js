    ...
    return this.onOffCluster[onoff ? 'setOn' : 'setOff']()
      .then(async result => {
        if (onoff === false) {
          ...
          // Wait for the device to settle, it ramps to its on level over `onTransitionTime`.
          // A dim command from the same user action wins, and so does one still transitioning.
          // Checked again after the read, a dim command can come in while it is in flight.
