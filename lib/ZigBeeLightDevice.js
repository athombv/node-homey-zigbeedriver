	...
	async onMeshInit() {
		...
		this.registerMultipleCapabilities([onoffCapabilityDefinition, dimCapabilityDefinition], (valueObj, optsObj) => {
			...
			if (valueObj.hasOwnProperty('onoff')) {
				if (valueObj.hasOwnProperty('dim') && valueObj.onoff && valueObj.dim > 0) {
					...
					return this.node.endpoints[this.getClusterEndpoint('levelControl')].clusters.levelControl.moveToLevelWithOnOff({
            ...
            transitionTime: util.calculateZigBeeDimDuration(optsObj.dim, this.getSettings()),
					...
					return this.node.endpoints[this.getClusterEndpoint('onOff')].clusters.onOff[valueObj.onoff ? 'setOn' : 'setOff']()
					...
					return this.node.endpoints[this.getClusterEndpoint('onOff')].clusters.onOff.setOff();
				}
				...
				return this.node.endpoints[this.getClusterEndpoint('onOff')].clusters.onOff[valueObj.onoff ? 'setOn' : 'setOff']()
					.then(async () => {
						if (valueObj.onoff === false) {
							...
							const dimLevel = await this.node.endpoints[this.getClusterEndpoint('levelControl')].clusters.levelControl.readAttributes('currentLevel');
		...
		if (this.hasCapability('light_temperature')) {
			...
			if (typeof this._colorTempMin !== 'number') {
				try {
					this._colorTempMin = await this.node.endpoints[this.getClusterEndpoint('colorControl')].clusters.colorControl.readAttributes('colorTempPhysicalMinMireds');
			...
			if (typeof this._colorTempMax !== 'number') {
				try {
					this._colorTempMax = await this.node.endpoints[this.getClusterEndpoint('colorControl')].clusters.colorControl.readAttributes('colorTempPhysicalMaxMireds');
		...
		this.registerMultipleCapabilities(groupedCapabilities, (valueObj, optsObj) => {
			...
			if (valueObj.hasOwnProperty('light_hue') && valueObj.hasOwnProperty('light_saturation')) {
				...
				return this.node.endpoints[this.getClusterEndpoint('colorControl')].clusters.colorControl.moveToHueAndSaturation({
            ...
            transitionTime: this.getSetting('transition_time') ? Math.round(this.getSetting('transition_time') * 10) : 0,
				...
				return this.node.endpoints[this.getClusterEndpoint('colorControl')].clusters.colorControl
          .moveToColorTemperature({
            colorTemperature: Math.round(util.mapValueRange(0, 1, this._colorTempMin, this._colorTempMax, valueObj.light_temperature)),
            transitionTime: this.getSetting('transition_time') ? Math.round(this.getSetting('transition_time') * 10) : 0,
					});
				...
				return this.node.endpoints[this.getClusterEndpoint('colorControl')].clusters.colorControl.moveToHue({
